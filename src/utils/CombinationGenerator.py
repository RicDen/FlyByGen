...
def generate_parameter_combinations(parameter_ranges, increments):
    object_value_combo_dict = {}
    recursion = False
    for key, value in increments.items():
        if isinstance(value, dict):
            print("Going deeper")
            recursion = True
            object_value_combo_dict[key] = generate_parameter_combinations(parameter_ranges[key], increments[key])
        # else:


            # generate_parameter_combinations(parameter_ranges[key], increments[key])
    if (recursion):
        return generate_config_combinations(object_value_combo_dict)
    else:
        parameter_value_combinations =  generate_value_combo_list(parameter_ranges, increments)
        object_value_combo_dict = assign_value_names(parameter_value_combinations, parameter_ranges)
        return  object_value_combo_dict
